    ...
    <div className="relative w-full px-5 py-10 box-border">
      <Slider {...settings}>
        {trending_data.map((item) => (
          <div key={item.id} className="relative bg-white rounded-lg overflow-hidden shadow-md p-4 mx-3 my-4 transition-transform duration-300 ease-in-out transform hover:translate-y-[-10px] hover:shadow-lg">
            <div className="w-36 h-36 mx-auto mb-4 relative overflow-hidden rounded-full border-3 border-gray-300 shadow-sm">
              <Image
                src={item.src}
                alt={item.title}
                width={item.width}
      ...
      <div className="slick-prev absolute left-[-30px] top-1/2 transform -translate-y-1/2 w-12 h-12 bg-gray-800 text-white flex items-center justify-center rounded-full z-10">
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
      </div>
      <div className="slick-next absolute right-[-30px] top-1/2 transform -translate-y-1/2 w-12 h-12 bg-gray-800 text-white flex items-center justify-center rounded-full z-10">
